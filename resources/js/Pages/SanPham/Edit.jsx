...
export default function Edit({ sanPham }) {
    const { data, setData, put, processing, errors } = useForm({
        TENSP: sanPham.TENSP || '',
        DVT: sanPham.DVT || '',
        NUOCSX: sanPham.NUOCSX || '',
        GIA: sanPham.GIA ? Math.floor(sanPham.GIA) : '',
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('san-pham.update', sanPham.MASP));
    };
        ...
        <AdminLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    Chỉnh Sửa Sản Phẩm: {sanPham.MASP}
                </h2>
            }
        >
                            ...
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        Không thể thay đổi mã sản phẩm
                                    </p>
                                </div>
